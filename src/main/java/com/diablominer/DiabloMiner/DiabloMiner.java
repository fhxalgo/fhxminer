...
public class DiabloMiner {
    private static final Logger log = LoggerFactory.getLogger(DiabloMiner.class);
	...
	void execute(String[] args) throws Exception {
		...
		for(int i = 0; j < networkStatesCount; i++, j++) {
            ...
            log.info("NetworkState: " + networkState);
            log.info("NetworkState: queryUrl: " + networkState.getQueryUrl());
        ...
        // main loop of calculating hashing output
		while(running.get()) {
            ...
            //System.out.println("XXXX hashCount: " + currentHashCount);
			...
			if(now - startTime > TIME_OFFSET * 2) {
				...
				if(!debug) {
					...
					for(List<? extends DeviceState> deviceStates : allDeviceStates) {
						for(DeviceState deviceState : deviceStates) {
							hashMeterFormatter.format("(%s: Gh %.1f basis: %.2f) ", deviceState.getDeviceName(), deviceState.getDeviceHashCount() / 1000.0 / 1000.0 / 1000.0, deviceState.getBasis());
    ...
    public static void main(String[] args) throws Exception {
        ...
        args = new String[]{"-l", "http://gfeng:br001klyn@localhost:18332", "-d", "-dt"};
        //System.setProperty("java.library.path", "C:/software/DiabloMiner/target/libs/natives");
